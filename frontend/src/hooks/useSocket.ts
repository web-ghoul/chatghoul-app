...
export function useSocket() {
  ...
  const handleNewMessage = useCallback(
    (message: Message) => {
      ...
      if (message.room) {
        messageService
          .markAsDelivered(String(message.room))
  ...
  // Handle new room event (for new chats/groups)
  const handleNewRoom = useCallback(
    (room: any) => {
      console.log("New room received:", room);
      useChatsStore.getState().addRoom(room);
      playNotificationSound();
    },
    [playNotificationSound],
  );
