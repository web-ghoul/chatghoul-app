...
export class MessagesService {
  ...
  async sendMessage(
    ...
    if (room.type === 'group' && room.permissions?.sendMessages === 'admins') {
      const isAdmin =
        room.admins.some((id) => String(id) === String(senderId)) ||
        String(room.superAdmin) === String(senderId);
      if (!isAdmin) {
    ...
    // Initialize deliveredTo with sender and any online participants
    const deliveredTo = [new Types.ObjectId(senderId)];
    room.participants.forEach((pId) => {
      const pIdStr = String(pId);
      if (
        pIdStr !== String(senderId) &&
        this.chatGateway.isUserOnline(pIdStr)
      ) {
        deliveredTo.push(new Types.ObjectId(pIdStr));
      }
    });
    ...
    room.participants.forEach((pId) => {
      ...
      const sanitizedMsg = {
        ...
        sender: this.usersService.sanitizeUserProfile(
  ...
  async deleteMessage(messageId: string, userId: string) {
    ...
    if (
      room.pinnedMessages.some((pm) => String(pm.message) === String(messageId))
    ) {
      await this.roomModel.findByIdAndUpdate(room._id, {
