...
export class RoomsService {
  ...
  async createRoom(createRoomDto: CreateRoomDto, creatorId: string) {
    ...
    const uniqueParticipants = Array.from(
      new Set([...participants, String(creatorId)]),
    );
    ...
    // Notify participants about the new room
    const roomIdStr = room._id.toString();

    // Emit to each participant
    uniqueParticipants.forEach(async (pId) => {
      const pIdStr = String(pId);
      // Get room as it would appear to this specific user (sanitized)
      try {
        const sanitizedRoom = await this.getRoomForUser(roomIdStr, pIdStr);
        this.chatGateway.emitToUser(pIdStr, 'new_room', sanitizedRoom);
      } catch (err) {
        // Ignore errors for individual users
      }
    });

    return this.getRoomById(roomIdStr, creatorId);
  }

  // Helper to get sanitized room for a specific user
  async getRoomForUser(roomId: string, userId: string) {
    const room = await this.roomModel
      .findById(roomId)
      .populate(
        'participants',
        'name avatar status lastSeen about phone email settings contacts',
      )
      ...
      .populate({
        ...
        populate: {
          path: 'sender',
          select: 'name avatar about settings contacts',
        },
      })
      .populate(
        'pinnedMessages.pinnedBy',
        'name avatar about settings contacts',
      );
    ...
    roomObj.participants = roomObj.participants.map((p) =>
      this.usersService.sanitizeUserProfile(p, userId),
    );
    roomObj.admins = roomObj.admins.map((a) =>
      this.usersService.sanitizeUserProfile(a, userId),
    );
    if (roomObj.superAdmin) {
      roomObj.superAdmin = this.usersService.sanitizeUserProfile(
        roomObj.superAdmin,
        userId,
      );
    }
  ...
  async getUserRooms(userId: string) {
    ...
    const sanitizedRooms = rooms.map((room) => {
      ...
      roomObj.pinnedMessages?.forEach((pm) => {
        if (pm.pinnedBy) {
          pm.pinnedBy = this.usersService.sanitizeUserProfile(
            pm.pinnedBy,
            userId,
          );
        }
        if (pm.message && pm.message['sender']) {
          pm.message['sender'] = this.usersService.sanitizeUserProfile(
            pm.message['sender'],
            userId,
          );
        }
  ...
  async getRoomById(roomId: string, userId: string) {
    ...
    room.admins = room.admins.map((a) =>
      this.usersService.sanitizeUserProfile(a, userId),
    ) as any;
    if (room.superAdmin) {
      room.superAdmin = this.usersService.sanitizeUserProfile(
        room.superAdmin,
        userId,
      );
    }
    ...
    const now = new Date();
    const activePins = room.pinnedMessages.filter(
      (pm) => !pm.expiresAt || pm.expiresAt > now,
    );
    ...
    if (activePins.length !== room.pinnedMessages.length) {
      ...
      await this.roomModel.updateOne(
        ...
        {
          $set: {
            pinnedMessages: activePins.map((pm) => ({
              ...pm,
              message: pm.message['_id'] || pm.message,
              pinnedBy: pm.pinnedBy['_id'] || pm.pinnedBy,
            })),
          },
        },
      );
  ...
  async pinMessage(
    ...
    if (room.type === 'group') {
      const isAdmin =
        room.admins.some((id) => String(id) === String(userId)) ||
        String(room.superAdmin) === String(userId);
      if (!isAdmin) {
  ...
  async unpinMessage(roomId: string, messageId: string, userId: string) {
    ...
    if (room.type === 'group') {
      ...
      if (!isAdmin) {
        throw new ForbiddenException(
          'Only admins can unpin messages in this group',
        );
      }
    }
  ...
  async changeSuperAdmin(
    ...
    if (
      !room.participants.some((id) => String(id) === String(newSuperAdminId))
    ) {
      throw new BadRequestException(
        'New Super Admin must be a participant of the room',
      );
    }
  ...
  async uploadFile(roomId: string, userId: string, file: Express.Multer.File) {
    ...
    const folderType = this.cloudinaryService.getFolderTypeFromMimetype(
      file.mimetype,
    );
  ...
  async deleteRoom(roomId: string, userId: string) {
    ...
    if (room.type === 'private') {
      ...
      if (String(room.superAdmin) === String(userId)) {
        ...
        throw new BadRequestException(
          'You are the Super Admin. Transfer ownership before exiting.',
        );
      }
  ...
  async updateRoom(
    ...
    if (updateRoomDto.permissions) {
      ...
      room.permissions = {
        ...((room.permissions as any)?.toObject?.() || room.permissions),
        ...updateRoomDto.permissions,
      };
    }
  ...
  async addParticipants(roomId: string, userIds: string[], userId: string) {
    ...
    const participantsToAdd = newUserIds.filter(
      (id) => !existingParticipantIds.includes(String(id)),
    );
